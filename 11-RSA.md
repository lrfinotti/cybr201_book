...
Before we introduce a new and important public-key cryptosystem, we need some theory.  We start by reviewing some facts about $\Z/m\Z$ from [](#05-Powers.md):
...
In this chapter we will deal with a modulus $N = pq$, where $p$ and $q$ are two distinct (*very* large) primes.  So, we have that $\varphi(N)=(p-1)(q-1)$.  Then, by [{name}](#th-euler), we have that if $\gcd(a, N) = 1$, then $a^{(p-1)(q-1)} = 1$ in $\Z/N\Z$.  But we can do better:

:::{prf:theorem} Euler's Formula
:label: th-order_pq
:numbered: true

Let $p$ and $q$ be distinct primes, $N = pq$, and $g = \gcd((p-1), (q-1))$, and $a \in (\Z/N\Z)^{\times}$.  Then, $a^{(p-1)(q-1)/g} = 1$ (in $\Z/N\Z$).  In particular, if both $p$ and $q$ are odd, we have that $a^{(p-1)(q-1)/2} = 1$.
:::

:::{prf:proof}
:numbered: false

Since $\gcd(a, pq) = 1$ (as $a$ is a unit in $\Z/N\Z$), we have that $\gcd(a, p) = \gcd(a, q) = 1$, so $a$ (as an integer) is also a unit ins $\Z/p\Z$ and $\Z/q\Z$.  Then, by [{name}](#th-eule), and noting the $(p-1)/g$ and $(q-1)/g$ are both *integers*, we have that
\begin{align*}
...
Since $\gcd(p, q) = 1$, this means that $pq \mid a^{(p-1)(q-1)/g}$, i.e., $a^{(p-1)(q-1)/g} = 1$ in $\Z/N\Z$.
...
Given $p$ and $q$ two distinct and large problems, $c \in \Z/N\Z$, and a positive integer $e$, how can we find $x$ such that $x^e = c$ (in $\Z/N\Z$), i.e., find the $e$-th root of $c$ (in $\Z/N\Z$)?
:::

We start approaching a simpler problem, when the modulus is *prime*:

:::{prf:proposition} Roots in $\Z/p\Z$
:label: prop-root_mod_p
:numbered: true

Let $p$ be prime, $c \in \Z/p\Z$, and $e$ a positive integer with $\gcd(e, p-1) = 1$.  Then, let $d$ be the inverse of $e$ modulo $p-1$ (i.e., $ed \equiv 1 \pmod{p-1}$).  Then
$$
x^e = c \quad \text{if and only if} \quad x = c^d.
$$
(So, $x$ is an $e$-th root of $c$ in $\Z/p\Z$.)
:::
...
So, assume that $x \neq 0$ (in $\Z/p\Z$), i.e., $x \in (\Z/p\Z)^{\times}$.  Since $\varphi(p) = p-1$, by [{name}](#th-euler) we have that $x^{p-1} = 1$.  Since $(p-1) \mid (de - 1)$, we have that $x^{de-1} = 1$, i.e., $x^{de} = x$.  Since $c$ is also a unit, the same argument tells us that $c^{de} = c$.
...
:::{prf:proposition} Roots in $\Z/pq\Z$
:label: prop-root_mod_pq
:numbered: true

Let $p$ and $q$ be dissecting primes, $N=pq$, $c \in \Z/N\Z$, and $e$ a positive integer with $\gcd(e, \varphi(N)) = 1$.  Then, let $d$ be the inverse of $e$ modulo $\varphi(N)$ (i.e., $ed \equiv 1 \pmod{\varphi(N)}$).  Then
$$
x^e = c \quad \text{if and only if} \quad x = c^d.
$$
(So, $x$ is an $e$-th root of $c$ in $\Z/N\Z$.)
:::
...
So, if $x^e = c$ (in $\Z/N\Z$), by [](#prop-root_mod_p), we have
\begin{align*}
...
\end{align*}
which together, since $\gcd(p, q) = 1$, tell us that $x \equiv c^d \pmod{N}$ (as $N=pq$), i.e., $x = c^d$ in $\Z/N\Z$.

Conversely, if $x=c^d$, then raising to the $e$-th power and since $ed \equiv 1 \pmod{\varphi(N)}$, we get by [{name}](#th-euler) and [](pr-power_eq_1) that $x^e = (c^d)^e = c^{de} = c$ in $\Z/N\Z$.
:::


The following proposition, gives a faster way to find roots in $\Z/N\Z$:

:::{prf:proposition}
:label: pr-root
:numbered: true

Let $p$ and $q$ be distinct primes, $N = pq$, $g = \gcd(p-1, q-1)$, $e$ a positive integer, and $d$ and inverse of $e$ modulo $(p-1)(q-1)/g$.  Then, for all $c \in $(\Z/N\Z)^{\times}$, we have that $x = c^d$ is such that $x^e = c$, i.e., $c^d$ is an $e$-th root of $c$ in $\Z/N\Z$.
:::
...
So, if we have $N = pq$ as above, $e$ a positive integer, and $c \in (\Z/N\Z)^{\times}$, and we want to solve $x^e = c$, we simply need to find and inverse of $e$ modulo $\varphi(N)$ (or modulo $(p-1)(q-1)/g$, where $g = \gcd(p-1, q-1)$).  So, we need to find $p$ and $q$ for the given $N$.
...
While the security of ElGamal's is based on the difficulty of computing discrete logs, RSA's security is based on the difficulty of taking $e$-th roots in $\Z/N\Z$, where $N$ is a product of very large primes, i.e., it's based on how hard it is to factor $N$.
...
2) **Encryption:** To send a message $m$ (a number in $\{2, 3, \ldots, N-1\}$) to Alice, Bob computes $c=m^e$ in $\Z/N/Z$, and sends $c$ to Alice.
3) **Decryption:**  Alice decodes the encrypted message $c$ by computing $c^d$ in $\Z/N\Z$.

Note that we know that the decryption works, since $de \equiv 1 \pmod{(p-1)(q-1)}$ implies that
$$
c^d = \left( m^e \right) ^d = m^{de} = m \quad \text{(in $\Z/N\Z$).}
$$
...
2) Encrypting and decrypting are done by computing powers in $\Z/N\Z$, so [Fast Powering](./05-Powers.md#fast-powering) comes handy!
...
5) Although we only know how to compute roots in $\Z/N\Z$ if we know how to factor $N$, it is possible that there might be a yet unknown, novel way without it.  Therefore, strictly speaking, the security of the RSA relies in the difficulty in computing roots in $\Z/N\Z$ (quickly enough), and not (necessarily) of the difficulty of factoring $N$.
:::
...
Let's illustrate how this could be done with the [Diffie-Hellman key exchange](./07-DH_and_ElGamal.md#DH_key_exchange):  Eve knows the prime $p$, $g \in \Fpt$, and its order $N$, as they are publicly available.
1) Alice chooses a secret key $a$, and sends $A = g^a$ (computed in $\F_p$) to Bob.
...
But imagine that Eve intercepts some message encrypted message $c = m^e \in \Z/N\Z$, using RSA with encrypting exponent $e$, from Bob to Alice.  Eve can make it look random by choosing an element $k \in (\Z/N\Z)^{\times}$ different from $0$ and $1$, and sending $c' = k^e c$, where $e$ is the public encrypting exponent.  She can then send it to Alice to verify she can decrypt it.  When she does, she computes
$$
...
But, when Eve received $km$, she can multiply it by $k^{-1}$ (the inverse in $\Z/N\Z$) and obtain $m$ back: $k^{-1} \cdot (km) = (k^{-1}k) \cdot m = 1 \cdot m = m$.  Then, Eve is capable of reading Bob's secrete message $m$, without really computing an $e$-th root in $\Z/N\Z$.
...
Suppose that Alice publishes two different encrypting exponents, say $e_1$ and $e_2$, for the same modulus $N$.  If somehow Eve can read the *same message from Bob*, say $c_1 = m^{e_1}$ and $c_2 = m^{e_2}$ (in $\Z/N\Z$), using the different exponents and $\gcd(e_1, e_3) = 1$, then Even can recover $m$: using the Extended Euclidean Algorithm, Eve finds integer $u$ and $v$ such that
$$
ue_1 + ve_2 = 1.
$$
